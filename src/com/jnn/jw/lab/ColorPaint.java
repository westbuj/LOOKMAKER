...
public class ColorPaint extends Activity {
	...
	public static ProgressDialog mProgress;//=new ProgressDialog(ColorPaint.mContext);
	
	public static Handler showProgress = new Handler() {

		public void handleMessage(Message msg) {
			ColorPaint.mProgress.setMessage("....downloading....");
			ColorPaint.mProgress.show();
		}
	};
	...
	public static Handler showSharing = new Handler() {
		...
		public void handleMessage(Message msg) {
			ColorPaint.mProgress.setMessage("....preparing....");
			ColorPaint.mProgress.show();
		}
	};
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mContext = this;
		mProgress=new ProgressDialog(ColorPaint.mContext);
		
		 	        
	        
		ColorPaint.mQueue.start();
		
		StateObjectWrapper so = (StateObjectWrapper)getLastNonConfigurationInstance();
		if (so!=null)
		{
			ColorPaint.cKit=so.cKit;
			ColorPaint.model=so.model;
			ColorPaint.modelBrowse=so.modelBrowse;
			ColorPaint.modelDirectory=so.modelDirectory;
			ColorPaint.modelsLoaded=so.modelsLoaded;
			ColorPaint.rawTree=so.rawTree;
			ColorPaint.tempHolder=so.tempHolder;
			ColorPaint.LOAD_DEFAULT=so.LOAD_DEFAULT;
		}
		else
			{LOAD_DEFAULT=true;
	...
	{   
		...
		so.cKit=ColorPaint.cKit;
		so.model=ColorPaint.model;
		so.modelBrowse=ColorPaint.modelBrowse;
		so.modelDirectory=ColorPaint.modelDirectory;
		so.modelsLoaded=ColorPaint.modelsLoaded;
		so.rawTree=ColorPaint.rawTree;
		so.tempHolder=ColorPaint.tempHolder;
		so.LOAD_DEFAULT=ColorPaint.LOAD_DEFAULT;
...

	{
		try {
			...
			case StaticResources.MENU_SHARE:
				...
				ColorPaint.showSharing.sendEmptyMessage(0);
				ColorPaint.mQueue.addTask(new ShareThread(ColorPaint.mContext));
				//ColorPaint.hideProgress.sendEmptyMessage(0);
				return true;
	...
	public static void loadmodelrules(ModelEntry m) {
		parseModelRules(ANR.getFile(ColorPaint.mContext.getResources(),m.mKitURL));
		...
		try {
			...
			for (int j = 0; j < e.length; j++) {
				...
				if (n.getTagName().equals("modelimage")) {
					...
					model = ANR.getBitmap(ColorPaint.mContext, n
							.getAttribute("src"));
					tempHolder = model;
					//i.setImageBitmap(model);
					//i.invalidate();
				...
				if (n.getTagName().equals("layer")) {// Load the layer
					...
					//		oPal.addPigment(k,"Remove "+ palette.getAttribute("Name"),255,255,255);
						...
						if (palette.getTagName().equals("externalPalette")) {
							Palette sPal = null;
							String exPallet = ANR.getFile(ColorPaint.mContext.getResources(), palette
									.getAttribute("src"));
			...
			ColorPaint.cKit = newKit;
			ColorPaint.loadKit();

		} catch (Exception e) {
			ANR.alert(ColorPaint.mContext, e.getMessage());
			return;
		}
	}
	...
	public static void loadKit() {
		...
		for (KitEntry k : ColorPaint.cKit) {
		...
		ColorPaint.rawTree = null;
		ColorPaint.rawTree = newTree;
	}
	...
	public static void renderImage() {
		// if (!ColorPaint.isRendering) return;
//	mRT.setPriority(Thread.MAX_PRIORITY);
		...
		int height = ColorPaint.model.getHeight();
		int width = ColorPaint.model.getWidth();
		// Bitmap newImage=ColorPaint.model.copy(Bitmap.Config.RGB_565 ,
		// true);// Bitmap.createBitmap(width, height,Bitmap.Config.RGB_565 );
		int[] newImage = getBitArray(ColorPaint.model);

		for (KitEntry ke : ColorPaint.cKit) {

			if (ColorPaint.resetRendering) {
				ColorPaint.resetRendering = false;
			...
			if (targetLayerColor != white) {
				// Bitmap mask=ke.getMask(ColorPaint.mContext);
				int[] mask = getBitArray(ke.getMask(ColorPaint.mContext));
		...
		ColorPaint.tempHolder = (Bitmap.createBitmap(newImage, width, height,
				Bitmap.Config.RGB_565));
		ColorPaint.updateImage.sendEmptyMessage(0);
		ColorPaint.isRendering = false;

	}
 
	public static void updateImage() {
		ColorPaint.i.setImageBitmap(tempHolder);
		ColorPaint.i.invalidate();
	}
	...
	public static void NotifyReload() {
		ColorPaint.i.setImageBitmap(model);
		ColorPaint.i.invalidate();
	}
